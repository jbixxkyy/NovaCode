...
export function sessionMessagePartID(messageID: string, type: "text" | "reasoning" | "file", ordinal: number) {
...
function assistantParts(sessionID: string, message: SessionMessageAssistant): Part[] {
  const ordinals = { text: 0, reasoning: 0, file: 0 }
...
function toolPart(sessionID: string, messageID: string, tool: SessionMessageAssistantTool): ToolPart {
  ...
  const state = (() => {
    ...
    const rawState = tool.state as typeof tool.state & { attachments?: Array<{ uri: string; mime: string; name?: string }> }
    const contentFiles = tool.state.content.flatMap((item, index): FilePart[] =>
      item.type === "file"
        ? [
            {
    ...
    const attachmentFiles: FilePart[] = (rawState.attachments ?? []).map((item, index) => ({
      id: `${tool.id}:att:${contentFiles.length + index}`,
      sessionID,
      messageID,
      type: "file" as const,
      mime: item.mime,
    ...
    const attachments = [...contentFiles, ...attachmentFiles]
    return {
