...
export function resolveChannel(): Channel {
  const raw = Bun.env.NOVACODE_CHANNEL ?? Bun.env.OPENCODE_CHANNEL
...
export async function downloadCliToResources() {
  ...
  const directory = await mkdtemp(join(tmpdir(), "novacode-cli-"))
  const dest = windowsify("resources/novacode-cli")
  try {
    ...
    try {
      ...
      console.warn(`Failed to copy CLI binary: ${e}`)
    }
  } finally {
