...
describe("hasExistingAppState", () => {
  ...
  test("recognizes state written by an earlier NovaCode launch", () => {
    expect(hasExistingAppState([file("novacode.settings")])).toBe(true)
    expect(hasExistingAppState([file("novacode.global.dat")])).toBe(true)
    expect(hasExistingAppState([file("window-state-abc.json")])).toBe(true)
    expect(hasExistingAppState([directory("novacode")])).toBe(true)
  })
})
