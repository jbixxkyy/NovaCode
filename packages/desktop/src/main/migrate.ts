...
function tauriAppId() {
  return app.isPackaged ? TAURI_APP_IDS[CHANNEL] : "ai.novacode.desktop.dev"
}

// Migrate a single Tauri .dat file into the corresponding electron-store.
// `novacode.settings.dat` is special: it maps to the `novacode.settings` store
...
// renderer already passes via IPC (e.g. `"default.dat"`, `"novacode.global.dat"`).
function migrateFile(datPath: string, filename: string) {
  ...
  // novacode.settings.dat → the electron settings store ("novacode.settings").
  // All other .dat files keep their full filename as the store name so they match
  // what the renderer passes via IPC (e.g. "default.dat", "novacode.global.dat").
  const storeName = filename === "novacode.settings.dat" ? "novacode.settings" : filename
