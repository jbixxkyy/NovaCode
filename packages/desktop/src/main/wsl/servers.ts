...
import { expectNovacodeVersion, pendingRestartAfterWslInstall, wslServerIdsToStartOnInitialize } from "./startup"
...
type WslServersControllerOptions = {
  ...
  resolveOpencode?: typeof resolveWslNovacode
...
export function createWslServersController(
  ...
  const checkOpencode = async (distro: string, opts?: { signal?: AbortSignal }) => {
    const resolved = await (options?.resolveOpencode ?? resolveWslNovacode)(distro, opts)
  ...
  return {
    ...
    async installOpencode(name: string) {
      await runJob({ kind: "install-opencode", distro: name, startedAt: Date.now() }, async (abort) => {
        const result = await installWslNovacode(appVersion, name, { signal: abort.signal })
        if (result.code !== 0) {
          throw new Error(summarize(result.stderr || result.stdout) || nativeT("desktop.wsl.error.installOpencode"))
        }
        await refreshOpencodeCheck(name, { signal: abort.signal })
        expectNovacodeVersion(state.opencodeChecks[name]?.version ?? null, appVersion, name)
