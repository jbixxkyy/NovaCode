...
let releaseNovacodeResolve: (() => void) | undefined
...
test("rejects an update that did not install the desktop version", () => {
  expect(() => expectNovacodeVersion("1.16.2", "1.16.2")).not.toThrow()
  expect(() => expectNovacodeVersion("1.14.35", "1.16.2")).toThrow(
    "NovaCode update finished but Debian still reports 1.14.35; expected 1.16.2",
  )
})

test("restarts an existing distro server after updating NovaCode", () => {
  expect(
    wslServerIdToRestart(
      [
        {
  ...
  expect(
    clearWslDistroState(
      { Debian: { name: "Debian", canExecute: true, hasBash: true, hasCurl: true, error: null } },
      {
        Debian: {
          distro: "Debian",
          resolvedPath: "/home/luke/.novacode/bin/novacode",
          version: "1.16.2",
...
test("ignores stale background NovaCode checks after removing a WSL server", async () => {
  persistedServers = []
  releaseNovacodeResolve = undefined
  ...
  await waitFor(() => !!releaseNovacodeResolve)
  await controller.removeServer("wsl:Debian")
  releaseNovacodeResolve?.()
...
test("ignores stale startup NovaCode checks after removing a WSL server", async () => {
...
test("probes addable distros in parallel before checking NovaCode", async () => {
  persistedServers = []
  ...
  const controller = createWslServersController("1.16.2", async () => new Promise<never>(() => undefined), {
    ...
    resolveOpencode: async (distro) => {
      ...
      return "/home/me/.novacode/bin/novacode"
    },
  })
...
test("does not check NovaCode in addable distros that cannot execute commands", async () => {
  persistedServers = []
  const opencode: string[] = []
  const controller = createWslServersController("1.16.2", async () => new Promise<never>(() => undefined), {
    ...testControllerOptions(),
  ...
  return {
    ...
    resolveOpencode: async () => {
      await new Promise<void>((resolve) => {
        releaseNovacodeResolve = resolve
      })
      return "/home/me/.novacode/bin/novacode"
    },
  }
