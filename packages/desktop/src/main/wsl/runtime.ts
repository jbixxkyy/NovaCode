...
export async function installWslNovacode(version: string, distro: string, opts?: RunWslOptions) {
  return runInteractiveCommand(
    resolveSystem32Command("wsl.exe"),
    wslArgs(
...
export async function resolveWslNovacode(distro: string, opts?: RunWslOptions) {
  return firstLine(
    (
      await runWslSh(
        [
