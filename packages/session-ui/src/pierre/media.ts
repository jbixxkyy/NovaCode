...
export function dataUrlFromMediaValue(value: MediaValue, kind: MediaKind) {
  ...
  // SVG files are served as text (type: "text") with no mime – handle directly
  if (kind === "svg" && record.type === "text") {
...
export function svgTextFromValue(value: MediaValue) {
  ...
  // SVG served as text (type: "text") – return directly for .svg files
  if (record.type === "text") return record.content
