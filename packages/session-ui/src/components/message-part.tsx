...
function syntheticFilePartsForTool(part: ToolPart): FilePart[] {
  if (part.state.status !== "completed") return []
  const attachments = (part.state as unknown as { attachments?: FilePart[] }).attachments
  if (!attachments?.length) return []
  return attachments
    .filter((item) => item.mime?.startsWith("image/"))
    .map((item, index) => ({
      ...item,
      id: `${part.id}:synthetic-file:${index}`,
      type: "file" as const,
...
export function expandToolAttachments(parts: PartType[]): PartType[] {
  const existingIds = new Set(parts.map((p) => p.id))
  const extra: PartType[] = []
  for (const part of parts) {
    if (part.type !== "tool") continue
    for (const file of syntheticFilePartsForTool(part as ToolPart)) {
      if (existingIds.has(file.id)) continue
      // avoid duplicating a server-provided file part with same url
      if (parts.some((p) => p.type === "file" && (p as FilePart).url === file.url)) continue
      existingIds.add(file.id)
      extra.push(file)
    }
  }
  if (extra.length === 0) return parts
  // keep original order, append synthetic files after their tool, preserving overall sort by id for groupParts
  // insert each synthetic file immediately after its parent tool in the array
  const result: PartType[] = []
  for (const part of parts) {
    result.push(part)
    if (part.type === "tool") {
      for (const file of extra) {
        if ((file as FilePart).id.startsWith(`${part.id}:synthetic-file:`)) result.push(file)
      }
    }
  }
  // push any remaining (shouldn't happen)
  for (const file of extra) if (!result.includes(file)) result.push(file)
  return result
}
    ...
    () =>
      groupParts(
        props.messages.flatMap((message) =>
          expandToolAttachments(list(data.store.part?.[message.id], emptyParts))
...
export function AssistantMessageDisplay(props: {
  ...
  const grouped = createMemo(
    () =>
      groupParts(
        expandToolAttachments(props.parts)
          .filter((part) => renderable(part, props.showReasoningSummaries ?? true))
          .map((part) => ({
...
export function ContextToolGroup(props: {
  ...
  return (
    ...
    >
      ...
      <Show
        when={(() => {
          const files = props.parts.flatMap((p) =>
            p.state.status === "completed"
              ? ((p.state as unknown as { attachments?: FilePart[] }).attachments ?? []).filter((a) =>
      ...
      >
        {(files) => (
          <div data-slot="context-tool-attachments">
            <For each={files()}>
              {(file) => (
                <img
                  data-slot="context-tool-attachment-image"
                  src={file.url}
                  alt={file.filename ?? i18n.t("ui.message.attachment.alt")}
                  onClick={() => dialog.show(() => <ImagePreview src={file.url} alt={file.filename} />)}
                />
              )}
            </For>
          </div>
        ...
        <Switch>
          ...
          <Match when={true}>
            <Dynamic
              ...
              onContentRendered={props.onContentRendered}
            />
            <Show when={part().state.status === "completed" && (part().state as unknown as { attachments?: FilePart[] }).attachments?.length}>
              <div data-slot="tool-attachments">
                <For each={(part().state as unknown as { attachments?: FilePart[] }).attachments}>
                  {(attachment) => (
                    <Show when={attachment.mime.startsWith("image/")}>
                      <img
                        data-slot="tool-attachment-image"
                        src={attachment.url}
                        alt={attachment.filename ?? i18n.t("ui.message.attachment.alt")}
                        onClick={() => dialog.show(() => <ImagePreview src={attachment.url} alt={attachment.filename} />)}
                      />
                    </Show>
                  )}
                </For>
...
PART_MAPPING["file"] = function FilePartDisplay(props) {
  ...
  return (
    <div data-component="file-part" data-timeline-part-id={part().id}>
      <div data-slot="file-part-body">
        <Show
          when={type() === "image"}
          fallback={
            <div data-slot="file-part-file" title={name()}>
              <FileIcon node={{ path: name(), type: "file" }} />
        ...
        >
          <div
            ...
            onClick={openPreview}
          >
            <img data-slot="file-part-image-tag" src={part().url} alt={name()} />
          </div>
        </Show>
      </div>
    </div>
