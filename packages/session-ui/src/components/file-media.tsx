...
export function FileMedia(props: { media?: FileMediaOptions; fallback: () => JSX.Element }) {
  ...
  const [svgView, setSvgView] = createSignal<"preview" | "code">("preview")
        ...
        {(() => {
          ...
          return (
            <div class="flex flex-col">
              <div class="flex items-center justify-center gap-1 px-6 py-2 bg-background-stronger border-b border-border-weak-base">
                <button
                  type="button"
                  class={`px-3 py-1 rounded text-12-medium transition-colors ${svgView() === "preview" ? "bg-background-base text-text-strong shadow-sm border border-border-weak-base" : "text-text-weak hover:text-text-strong"}`}
                ...
                <button
                  type="button"
                  class={`px-3 py-1 rounded text-12-medium transition-colors ${svgView() === "code" ? "bg-background-base text-text-strong shadow-sm border border-border-weak-base" : "text-text-weak hover:text-text-strong"}`}
                  onClick={() => setSvgView("code")}
                >
                  Code
                </button>
              </div>
              <Show when={svgView() === "preview"} fallback={props.fallback()}>
                <div class="flex justify-center bg-background-stronger px-6 py-4">
                  <img
                    src={src}
                    alt={cfg()?.path}
