...
function globalConfigFile() {
  const candidates = ["novacode.jsonc", "novacode.json", "opencode.jsonc", "opencode.json", "config.json"].map((file) =>
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadGlobal = Effect.fnUntraced(function* (env?: Record<string, string>) {
      ...
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "novacode.json"), env))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "novacode.jsonc"), env))
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        if (existsSync(managedDir)) {
          for (const file of ["opencode.json", "opencode.jsonc", "novacode.json", "novacode.jsonc"]) {
